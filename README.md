...
6. Launch CDMetaPOP: There are a number of ways to run CDMetaPOP. If you are using a command shell you can run the program by typing “python CDMetaPOP.py C:/”homedirectorylocation”/example_files RunVars.csv output_test”. Or a short-cut if your data is located at the same folder level as the src folder: “python CDMetaPOP.py ../example_files/ PopVars.csv output_test”. Note that there are 5 arguments here that must be included with spaces in between: 
