	...
	for imc in xrange(mcno):
		...
		for iout in xrange(gen):
			...
			for j in xrange(len(values[1+iout][2].split('|'))-1):
				...
				if values[1+iout][11].split('|')[j] == 'NA':
					PackDeaths_Emi[ibatch][imc][iout].append(nan)
				else:
					PackDeaths_Emi[ibatch][imc][iout].append(int(values[1+iout][11].split('|')[j]))
				if values[1+iout][18].split('|')[j] == 'NA':
					PackDeaths_Immi[ibatch][imc][iout].append(nan)
				else:
					PackDeaths_Immi[ibatch][imc][iout].append(int(values[1+iout][18].split('|')[j]))
