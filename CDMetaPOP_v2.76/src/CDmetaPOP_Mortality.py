...
def ConstantMortality_Add(SubpopIN,K,PopDeaths,age_percmort,pop_percmort,gen,Population,AgeDeaths,sizecall,size_mean,size_percmort,SizeDeaths,sexchromo):
	...
	for isub in range(len(K)):	
		...
		if pop_percmort[isub] == 'E' and Npop != 0:
			...
			# Then delete all
			deleteall = np.asarray(list(range(0,Npop,1)))
		...
		# Keep going if individuals in this patch
			...
			if pop_percmort[isub] == 'N' or pop_percmort[isub] == 0.0 or pop_percmort[isub] == '0' or pop_percmort[isub] == '0.0':
				...
				sample_survivors = np.asarray(np.random.choice(list(range(Npop)),Nsurvivors, replace=False))
			...
			if Nsurvivors != 0:
				...
				for ifile in range(len(countfiles[0])):
