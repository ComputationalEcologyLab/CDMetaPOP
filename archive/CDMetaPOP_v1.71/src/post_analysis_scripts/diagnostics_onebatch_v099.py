...
# _onebatch_v099: to use with one batch at a time and match v0.99
# ----------------------------------------------------------------------------- 
...
dir = "D:/projects/CDmetaPOP/NASAA36/Runs/dataLCT1278_v099_Master_20150320/LCT1278_20150404/stray2/"
plottitle = 'Stray Movement Inverse-square'
savename = 'Stray2_'
...
maxA = 11*17 # loci * alleles
mcno = 10 # Number of MCs
plottime = np.asarray([0,9,49,109])
# Plot time within: N_init, N_Emi, N_Emiplus0s, N_EmiMort, N_Immi, N_ImmiMort
withintime = frange(0,gen,1/6.)[0:-1]
time_init = nthfile
time_Emi = frange(0.17,gen-1+0.17,1)
time_Emi0 = frange(0.33,gen-1+0.33,1)
time_EmiMort = frange(0.5,gen-1+0.5,1)
time_Immi = frange(0.67,gen-1+0.67,1)
time_ImmiMort = frange(0.83,gen-1+0.83,1)
		...
		for iout in xrange(gen):
			...
			for j in xrange(len(values[1+iout][2].split('|'))-1):
				...
				if values[1+iout][18].split('|')[j] == 'NA':
					...
					N_afterImmi[ibatch][imc][iout].append(int(values[1+iout][18].split('|')[j]))
				...
				if values[1+iout][14].split('|')[j] == 'NA':
					N_growthout[ibatch][imc][iout].append(nan)
				else:
					N_growthout[ibatch][imc][iout].append(int(values[1+iout][14].split('|')[j]))
...
ylabel('Initial Population',fontsize=18)
...
for i in xrange(len(N_growth_patch_m)):
	plot(nthfile[1:],N_growth_patch_m[i],linemarks[i],label=label[i],ms=2)
xlabel('Time',fontsize=18)
